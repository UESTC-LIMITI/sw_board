...
uint8_t FDCAN_Init(FDCAN_HandleTypeDef *hfdcan)
{
	...
	if(hfdcan == &hfdcan1)
	{
		...
		if (HAL_FDCAN_ActivateNotification(hfdcan, FDCAN_IT_BUS_OFF, 0) != HAL_OK)               //激活FDCAN1的总线关闭中断
		{
			Error_Handler();
		}
		if (HAL_FDCAN_ActivateNotification(hfdcan, FDCAN_IT_RX_FIFO0_NEW_MESSAGE, 0) != HAL_OK)  //打开FDCAN1的接收中断
		{
			Error_Handler();
		}
